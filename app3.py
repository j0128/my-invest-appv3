...
def run_backtest_lab_v2(ticker, df_close, df_high, df_low, df_macro, f_data, days_ago=30):
    ...
    return {"Past_Pred": final_pred, "Present_Value": p_now, "Error": err, "Process": process, "Macro_State": macro_status}
...
def main():
    ...
    with t1:
        ...
        if df_macro is not None:
            ...
            try: liq_chg = liq - df_macro['Net_Liquidity'].iloc[-20]
            except: liq_chg = 0
        ...
        # 2. Strategy Table (The CFO Table)
        ...
        for t in tickers:
            ...
            try: mvrv = calc_mvrv_z(df_close[t]).iloc[-1] 
            except: mvrv = 0
            ...
            summary.append({
                ...
                "CFO 指令": act, "預期範圍(±2σ)": pred_range, "目標價(Avg)": f"${tgt_val:.2f}",
        ...
        for t in tickers:
            ...
            with st.expander(f"🦅 {t} 深度雷達掃描", expanded=False):
                k1, k2, k3 = st.columns([2, 1, 1])
                
                # 左側：槓桿對比
                with k1: 
                    if comp_res: 
                        st.plotly_chart(px.line(comp_res[0], title=f"{t} vs TQQQ (Normalized)").update_layout(height=300), use_container_width=True)
