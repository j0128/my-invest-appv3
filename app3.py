...
# --- 0. 全局設定 ---
st.set_page_config(page_title="Alpha 13.1: 混合指揮官 Pro", layout="wide", page_icon="🦅")
...
def analyze_trend_multi(series):
    ...
    sma200_prev = sma200_series.iloc[-10]
...
def calc_kelly(trend_status):
    ...
    return max(0, (win * 2.0 - 1) / 1.0 * 0.5) # Half-Kelly
...
def main():
    ...
    with st.spinner("🦅 Alpha 13.1 正在執行混合全域掃描..."):
    ...
    with t1:
        ...
        for t in tickers_list:
            ...
            with st.expander(f"🦅 {t} 戰略深度分析 (DCF/PE/Backtest)", expanded=False):
                ...
                with k3:
                    ...
                    st.write(f"DCF 估值: ${targets['DCF']:.2f}" if targets and targets['DCF'] else "N/A")
                    st.write(f"預估 PE: {adv_data.get(t,{}).get('Forward_PE') or '-'}")
                    # OBV 圖表
    ...
    with t7:
        ...
        lab_ticker = st.selectbox("標的", sorted(list(set(tickers_list + ['TQQQ', 'QQQ', 'SPY']))))
        if lab_ticker in df_close.columns:
            ...
            c1, c2, c3 = st.columns(3); c1.metric("總投入", f"${inv:,.0f}"); c2.metric("DCA", f"{r_dca:.1%}"); c3.metric("混合策略", f"{r_strat:.1%}")
