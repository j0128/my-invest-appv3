...
# --- 0. 全局設定 ---
st.set_page_config(page_title="Alpha 12.8: 全功能牛熊版", layout="wide", page_icon="🦅")
...
def run_strategy_backtest_salary_flow_v2(df_in, vol_in):
    ...
    for i in range(len(df)):
        ...
        if date.month != last_month: # 每月入金 10,000
            cash_dca += 10000; cash_strat += 10000; invested += 10000; last_month = date.month
            buy_dca = cash_dca // p; shares_dca += buy_dca; cash_dca -= buy_dca * p # DCA 即買
        if i > 20:
...
def main():
    with st.sidebar:
        ...
        user_input = st.text_area("持倉市值清單 (代號, 市值)", "BTC-USD, 10000\nAMD, 10000\nNVDA, 10000", height=150)
    ...
    with st.spinner("🦅 Alpha 12.8 正在全速運算..."):
    ...
    with t1:
        ...
        for t in tickers_list:
            ...
            cfo_act, build_pct = get_cfo_directive_v4(tr['p_now'], six_s, tr['status'], tr['is_bull'], rsi, slope, vol_r, mvrv_z, tgt*1.05, tgt*0.95)
            mode_tag = '<span class="bull-mode">BULL</span>' if tr['is_bull'] else '<span class="bear-mode">BEAR</span>'
            summary.append({"代號": t, "模式": mode_tag, "現價": f"${tr['p_now']:.2f}", "CFO 指令": cfo_act, "建議持倉": f"${(total_assets*slot_limit*build_pct):,.0f}", "預測值": f"${tgt:.2f}"})
        st.write(pd.DataFrame(summary).to_html(escape=False), unsafe_allow_html=True)
        
        st.markdown("---")
        st.subheader("2. 個股雷達 (還原詳細分析)")
        for t in tickers_list:
            if t not in df_close.columns: continue
    ...
    with t3:
        st.subheader("🔍 財務體質掃描")
        h_data = [{"代號": t, "PEG": f"{(adv_data.get(t,{}).get('PEG') or 0):.2f}", "ROE": f"{(adv_data.get(t,{}).get('ROE') or 0)*100:.1f}%", "淨利率": f"{(adv_data.get(t,{}).get('Profit_Margin') or 0)*100:.1f}%", "流動比": adv_data.get(t,{}).get('Current_Ratio') or "-", "負債權益": adv_data.get(t,{}).get('Debt_Equity') or "-"} for t in tickers_list]
        st.dataframe(pd.DataFrame(h_data), use_container_width=True)

    # === TAB 4 & 5 & 6: 回測/財報/房貸 (完整還原) ===
    with t4:
        st.subheader("🚦 SMA200 回測")
        for t in tickers_list:
            if t in df_close.columns:
                sma200 = df_close[t].rolling(200).mean(); sig = np.where(df_close[t] > sma200, 1, 0)
                strat = (1 + df_close[t].pct_change() * pd.Series(sig).shift(1).values).cumprod()
                bh = (1 + df_close[t].pct_change()).cumprod()
                st.write(f"**{t}**"); st.line_chart(pd.DataFrame({"策略": strat, "買入持有": bh}).dropna())
    ...
    # === TAB 7: 策略實驗室 (完整還原薪資流) ===
    with t7:
        st.subheader("📊 混合戰略實驗室 (Hybrid vs DCA)")
        lab_t = st.selectbox("標的", sorted(list(set(tickers_list + ['TQQQ', 'QQQ', 'SPY']))))
        if lab_t in df_close.columns:
            ...
            c1, c2, c3 = st.columns(3); c1.metric("投入本金", f"${inv:,.0f}"); c2.metric("DCA 淨值", f"${dca_f:,.0f}", f"{r_dca:.1%}"); c3.metric("混合策略 淨值", f"${strat_f:,.0f}", f"{r_strat:.1%}")
            fig = go.Figure(); fig.add_trace(go.Scatter(x=res.index, y=res['DCA'], name='DCA 薪資流', line=dict(dash='dash')))
            fig.add_trace(go.Scatter(x=res.index, y=res['Strat'], name='混合策略', line=dict(color='#00BFFF'))); st.plotly_chart(fig, use_container_width=True)
