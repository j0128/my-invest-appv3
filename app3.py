...
def fetch_fred_macro(api_key):
    ...
    這代表了市場 $H^0$ (Global Section) 的支撐力量。
    """
...
def run_strategy_backtest_salary_flow_v3(df_in, vol_in, df_macro):
    ...
    return res, final_dca, final_strat, invested, res['DCA'].iloc[-1], res['Strat'].iloc[-1]
...
def main():
    ...
    with st.spinner("🦅 Alpha 13.7 正在執行拓撲全域掃描 (含動態凱利權重)..."):
        # 獲取所有數據
    ...
    with t7:
        ...
        if lab_ticker in df_close.columns:
            ...
            c3.metric("混合策略 報酬率", f"{r_strat:.1%}", delta=f"{(r_strat-r_dca)*100:.1f} pts")
