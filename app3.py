...
def calc_targets_composite(ticker, df_close, df_high, df_low, f_data, days_forecast=22):
    ...
    try:
        ...
        t_dcf = (eps * (1 + growth)**5 * hist_pe) / (1.1**5) if eps > 0 else None
...
def main():
    ...
    with st.spinner("🦅 Alpha 13.1 正在進行全域掃描..."):
    ...
    with t7:
        ...
        lab_ticker = st.selectbox("選擇回測標的", sorted(list(set(tickers_list + ['TQQQ', 'SPY']))))
